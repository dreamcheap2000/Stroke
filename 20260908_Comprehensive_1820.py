#!/usr/bin/env python3
"""
20260908_Comprehensive_1820.py
======================
Combined bootstrap LASSO (no IPCW) + single-completion-model IPCW pipeline for
6MWT4 prediction.  This update carries forward the 20260904 comprehensive
analysis and adds a delta-adjusted pattern-mixture sensitivity analysis.
...
Outputs (written to repo root):
    20260908_Comprehensive_1820.docx  – combined LASSO + IPCW report
    20260908_Comprehensive_1820.xlsx  – full dataset with per-patient predictions

Author: auto-generated 2026-09-08
"""
...
OUTPUT_DOCX = ROOT / "20260908_Comprehensive_1820.docx"
OUTPUT_XLSX = ROOT / "20260908_Comprehensive_1820.xlsx"
...
BINARY_MODEL_NAME = "LogisticRegression(class_weight='balanced', solver='liblinear')"
DELTA_GRID = np.arange(-120, 121, 10, dtype="float64")
CLINICALLY_PLAUSIBLE_DELTA_ABS_MAX = 83.1
DELTA_TIPPING_CRITERION = (
    "First Δ where the within-scenario augmented weighted OOF R² rank changes from Δ=0"
)
...
def _build_binary_logistic_pipeline(features: list[str]) -> Pipeline:
...
def select_binary_classifier(df: pd.DataFrame, features: list[str], scenario: str) -> dict:
    ...
    pipe = _build_binary_logistic_pipeline(valid)
    ...
    pipe.fit(X, y)
    return {"best_model_name": BINARY_MODEL_NAME, "best_pipeline": pipe, "features": valid,
            "leaderboard": [row], "bal_acc": row["balanced_accuracy"]}
...
def _fit_ridge_oof(
    ...
    if len(X_raw) < 3:
        raise ValueError("Need at least 3 rows for cross-validated Ridge fitting.")
    n_splits = min(n_splits, len(X_raw) - 1)
    n_splits = max(2, n_splits)
    cv_splits = list(KFold(n_splits=n_splits, shuffle=True, random_state=RANDOM_STATE).split(X_raw))
    oof = np.zeros(len(X_raw), dtype="float64")
    for tr, te in cv_splits:
        imp = SimpleImputer(strategy="median")
        X_tr = imp.fit_transform(X_raw.iloc[tr])
        X_te = imp.transform(X_raw.iloc[te])
        ridge = Ridge(alpha=1.0)
        ridge.fit(X_tr, y[tr], sample_weight=sample_weight[tr])
        oof[te] = np.maximum(0, ridge.predict(X_te))
    return oof


def _shift_noncompleter_predictions(regression_result: dict, delta: float) -> np.ndarray:
    walk_pred = np.asarray(regression_result["noncomp_walk_pred"], dtype=int)
    base_pred = np.asarray(regression_result["noncomp_6mwt4_pred"], dtype="float64")
    shifted = np.zeros(len(base_pred), dtype="float64")
    if np.any(walk_pred == 1):
        shifted[walk_pred == 1] = np.maximum(0.0, base_pred[walk_pred == 1] + delta)
    return shifted


def _build_delta_analysis_dataset(
    df: pd.DataFrame,
    features: list[str],
    ipcw: dict,
    ...
    X_comp = df.loc[completer_mask, valid].copy()
    y_comp = df.loc[completer_mask, "6MWT4"].to_numpy(dtype="float64")
    w_comp = ipcw["weights"].loc[completer_mask].fillna(1.0).to_numpy(dtype="float64")

    X_nc = df.loc[noncomp_index, valid].copy()
    y_nc = _shift_noncompleter_predictions(regression_result, delta)
    w_nc = np.ones(len(X_nc), dtype="float64")

    X_all = pd.concat([X_comp, X_nc], axis=0).reset_index(drop=True)
    y_all = np.concatenate([y_comp, y_nc])
    w_all = np.concatenate([w_comp, w_nc])
    strata = np.concatenate([
        np.repeat("Completer", len(X_comp)),
        np.repeat("Non-completer", len(X_nc)),
    ])
    return X_all, y_all, w_all, strata


def evaluate_delta_adjusted_ridge(
    df: pd.DataFrame,
    features: list[str],
    ipcw: dict,
    regression_result: dict,
    ...
    X_all, y_all, w_all, strata = _build_delta_analysis_dataset(df, features, ipcw, regression_result, delta)
    oof = _fit_ridge_oof(X_all, y_all, w_all)
    return {
        "delta": float(delta),
        "weighted_r2": float(r2_score(y_all, oof, sample_weight=w_all)),
        "weighted_mae": float(np.average(np.abs(y_all - oof), weights=w_all)),
        "n_total": int(len(y_all)),
        "n_completers": int(np.sum(strata == "Completer")),
        "n_noncompleters": int(np.sum(strata == "Non-completer")),
        "mean_imputed_noncompleter_6mwt4": float(np.mean(y_all[strata == "Non-completer"])),
    }


def _stratified_resample_indices(strata: np.ndarray, rng: np.random.Generator) -> np.ndarray:
    idx_resampled: list[np.ndarray] = []
    for grp in pd.unique(strata):
        grp_idx = np.where(strata == grp)[0]
        idx_resampled.append(rng.choice(grp_idx, size=len(grp_idx), replace=True))
    return np.concatenate(idx_resampled)
...
def bootstrap_delta_r2(
    ...
    for _ in range(n_bootstrap):
        boot_idx = _stratified_resample_indices(strata, rng)
        X_boot = X_all.iloc[boot_idx].reset_index(drop=True)
        y_boot = y_all[boot_idx]
        w_boot = w_all[boot_idx]
        try:
            n_splits = min(CV_FOLDS, max(2, len(X_boot) // 10))
            oof_boot = _fit_ridge_oof(X_boot, y_boot, w_boot, n_splits=n_splits)
        except Exception:
            continue
        r2_boot.append(float(r2_score(y_boot, oof_boot, sample_weight=w_boot)))
        mae_boot.append(float(np.average(np.abs(y_boot - oof_boot), weights=w_boot)))
...
def _delta_implausibility_label(delta: float) -> str:
    if not math.isfinite(delta):
        return f"Not reached within explored grid ({int(DELTA_GRID.min())} to +{int(DELTA_GRID.max())} m)"
    if abs(delta) <= CLINICALLY_PLAUSIBLE_DELTA_ABS_MAX:
        return f"Within clinically plausible range (|Δ| ≤ {CLINICALLY_PLAUSIBLE_DELTA_ABS_MAX:.1f} m)"
    if delta < 0:
        return (
            f"Exceeds clinically plausible negative dropout-severity range "
            f"(|Δ| > {CLINICALLY_PLAUSIBLE_DELTA_ABS_MAX:.1f} m)"
        )
    return (
        f"Exceeds clinically plausible positive improvement range "
        f"(|Δ| > {CLINICALLY_PLAUSIBLE_DELTA_ABS_MAX:.1f} m)"
    )


def run_delta_tipping_analysis(
    df: pd.DataFrame,
    ipcw: dict,
    ...
    for res in ipcw_tier_results:
        for delta in DELTA_GRID:
            eval_res = evaluate_delta_adjusted_ridge(df, res["features"], ipcw, res["regression"], float(delta))
            grid_rows.append({
                "Model": res["model"],
                "Scenario": res["scenario_label"],
                "Delta_m": float(delta),
                "Augmented_Weighted_OOF_R2": round(eval_res["weighted_r2"], 4),
                "Augmented_Weighted_OOF_MAE": round(eval_res["weighted_mae"], 2),
                "Mean_Imputed_NonCompleter_6MWT4": round(eval_res["mean_imputed_noncompleter_6mwt4"], 2),
                "N_total": eval_res["n_total"],
                "N_completers": eval_res["n_completers"],
                "N_noncompleters": eval_res["n_noncompleters"],
            })

    grid_df = pd.DataFrame(grid_rows)
    if grid_df.empty:
        return grid_df, grid_df, grid_df

    grid_df["Rank_within_Scenario_at_Delta"] = (
        grid_df.groupby(["Scenario", "Delta_m"])["Augmented_Weighted_OOF_R2"]
        .rank(method="dense", ascending=False)
        .astype(int)
    )

    delta_order = sorted(
        [float(delta) for delta in DELTA_GRID if delta != 0],
        key=lambda val: (abs(val), 0 if val < 0 else 1),
    )
    summary_rows: list[dict] = []
    ...
    for res in ipcw_tier_results:
        model = res["model"]
        scenario = res["scenario_label"]
        model_grid = grid_df[(grid_df["Model"] == model) & (grid_df["Scenario"] == scenario)].copy()
        baseline_row = model_grid.loc[model_grid["Delta_m"] == 0].iloc[0]
        baseline_rank = int(baseline_row["Rank_within_Scenario_at_Delta"])
        tip_row = None
        for delta in delta_order:
            candidate = model_grid.loc[model_grid["Delta_m"] == delta]
            if candidate.empty:
                continue
            candidate_row = candidate.iloc[0]
            if int(candidate_row["Rank_within_Scenario_at_Delta"]) != baseline_rank:
                tip_row = candidate_row
                break

        baseline_boot = bootstrap_delta_r2(df, res["features"], ipcw, res["regression"], 0.0)
        ...
        tip_boot = {
            ...
            "r2_ci_lo": np.nan,
            "r2_ci_hi": np.nan,
            "mae_boot_mean": np.nan,
            "mae_ci_lo": np.nan,
            "mae_ci_hi": np.nan,
        }
        explainer = (
            f"Baseline within-scenario augmented R² rank was {baseline_rank}; "
            f"no rank change was observed across the explored Δ grid."
        )
        if tip_row is not None:
            tip_delta = float(tip_row["Delta_m"])
            tip_r2 = float(tip_row["Augmented_Weighted_OOF_R2"])
            tip_rank = int(tip_row["Rank_within_Scenario_at_Delta"])
            tip_boot = bootstrap_delta_r2(df, res["features"], ipcw, res["regression"], tip_delta)
            bootstrap_rows.append({
                "Model": model,
                "Scenario": scenario,
                "Delta_m": tip_delta,
            ...
            explainer = (
                f"Baseline within-scenario augmented R² rank was {baseline_rank}; "
                f"the first rank change occurred at Δ={tip_delta:.0f} m, where the model moved to rank {tip_rank}."
            )

        summary_rows.append({
            "Model": model,
            "Scenario": scenario,
            "Δ tipping point (m)": tip_delta,
            "R² at Δ=0": float(baseline_row["Augmented_Weighted_OOF_R2"]),
            "R² at tipping point": tip_r2,
            "Direction of implausibility": _delta_implausibility_label(tip_delta),
            "Baseline_rank_at_Δ0": baseline_rank,
            "Rank_at_tipping_point": tip_rank,
            "Tipping_criterion": DELTA_TIPPING_CRITERION,
            "R² at Δ=0 bootstrap mean": baseline_boot["r2_boot_mean"],
            "R² at Δ=0 CI lo": baseline_boot["r2_ci_lo"],
            "R² at Δ=0 CI hi": baseline_boot["r2_ci_hi"],
            "R² at tipping bootstrap mean": tip_boot["r2_boot_mean"],
            "R² at tipping CI lo": tip_boot["r2_ci_lo"],
            "R² at tipping CI hi": tip_boot["r2_ci_hi"],
            "Explainer": explainer,
        })

    summary_df = pd.DataFrame(summary_rows)
    bootstrap_df = pd.DataFrame(bootstrap_rows)
    return summary_df, grid_df, bootstrap_df
...
def fit_weighted_ridge(
    ...
    oof = _fit_ridge_oof(X_raw, y, weights, n_splits=CV_FOLDS)
...
def _bootstrap_ipcw_once(
    ...
    try:
        ...
        bin_pipe = _build_binary_logistic_pipeline(valid)
    ...
    try:
        ...
        oof = _fit_ridge_oof(X_raw, y, weights, n_splits=n_splits)
        r2 = float(r2_score(y, oof, sample_weight=weights))
        mae = float(np.average(np.abs(y - oof), weights=weights))
        return r2, mae
    except Exception:
...
def _fmt_delta_tip(delta: float) -> str:
    return "Not reached" if not math.isfinite(delta) else f"{delta:.0f} m"
...
def write_combined_report(
    ...
    title.add_run(
        "Comprehensive LASSO + IPCW Report for 6MWT4 Prediction – 2026-09-08"
    ).bold = True
    ...
    intro = doc.add_paragraph(
        ...
        "practice.\n"
        "(3) Delta-adjusted pattern-mixture sensitivity analysis reusing the existing "
        "stabilized weights and fixed logistic non-completer classification across the "
        "10 retained model/scenario runs."
    )
    _small_para(intro, 9)
    ...
    ipcw_intro = doc.add_paragraph(
        ...
        "• The non-completer walk/no-walk classifier is fixed to logistic regression "
        "for every retained model/scenario so that Part 2 comparisons are not driven "
        "by varying tree-based classifiers.\n\n"
    ...
    for res in ipcw_tier_results:
        ...
        reg = res["regression"]
        ci = res["bootstrap_ci"]
        delta_row = delta_lookup.get((res["model"], res["scenario_label"]))
        delta_rows = []
        if delta_row is not None:
            delta_rows = [
                ["Binary model", res["binary"]["best_model_name"] + " (fixed for consistency)"],
                ["Δ tipping criterion", delta_row["Tipping_criterion"]],
                ["Δ tipping point", _fmt_delta_tip(delta_row["Δ tipping point (m)"])],
                ["Augmented R² at Δ=0", _fmt(delta_row["R² at Δ=0"], 4)],
                ["Augmented R² at tipping point", _fmt(delta_row["R² at tipping point"], 4)],
                ["Implausibility assessment", delta_row["Direction of implausibility"]],
            ]
    ...
    if not delta_summary_df.empty:
        doc.add_paragraph()
        dh = doc.add_paragraph()
        dh.add_run("Delta-Adjusted Pattern-Mixture Tipping-Point Summary").bold = True

        delta_intro = doc.add_paragraph(
            f"Each retained IPCW model/scenario was re-evaluated over a symmetric Δ grid "
            f"({int(DELTA_GRID.min())} to +{int(DELTA_GRID.max())} m, 10 m steps). "
            "For each run, the existing stabilized IPCW weights for completers and the fixed "
            "logistic non-completer walk/no-walk classification were held constant; only the "
            "imputed 6MWT4 values of predicted-walking non-completers were shifted before "
            "re-fitting the weighted Ridge model. The tipping point is the first Δ at which "
            "the within-scenario augmented weighted OOF R² rank changes from its Δ=0 ordering. "
            f"Values beyond |Δ|={CLINICALLY_PLAUSIBLE_DELTA_ABS_MAX:.1f} m were flagged as clinically implausible "
            "relative to published 6MWT MCID benchmarks."
        )
        _small_para(delta_intro, 9)

        delta_rows = [[
            "Model", "Scenario", "Δ tipping point (m)", "R² at Δ=0",
            "R² at tipping point", "Direction of implausibility",
        ]]
        for _, row in delta_summary_df.iterrows():
            delta_rows.append([
                row["Model"],
                row["Scenario"],
                _fmt_delta_tip(row["Δ tipping point (m)"]),
                _fmt(row["R² at Δ=0"], 4),
                _fmt(row["R² at tipping point"], 4),
                row["Direction of implausibility"],
            ])
        _add_table_to_doc(doc, delta_rows)

        for _, row in delta_summary_df.iterrows():
            explainer = doc.add_paragraph(f"{row['Model']} | {row['Scenario']}: {row['Explainer']}")
            _small_para(explainer, 8)
    ...
    meth_items = [
        ...
        ("Fixed logistic non-completer classifier",
         "For consistency across the retained IPCW models, the walk/no-walk classifier for "
         "non-completers is fixed to class-balanced logistic regression rather than allowing "
         "tree-based alternatives such as RandomForest to vary by tier. This keeps the Part 2 "
         "comparison focused on the weighted Ridge outcome models rather than on classifier choice."),
        ...
        ("Delta-adjusted pattern-mixture sensitivity",
         "The tipping-point analysis reuses the already fitted stabilized weights and the fixed "
         "non-completer classification for each model/scenario, then shifts the imputed 6MWT4 values "
         f"of predicted-walking non-completers over Δ ∈ [{int(DELTA_GRID.min())}, +{int(DELTA_GRID.max())}] m. "
         f"We flagged tipping points beyond |Δ|={CLINICALLY_PLAUSIBLE_DELTA_ABS_MAX:.1f} m as clinically implausible, "
         "reflecting the upper end of published stroke-specific and cross-condition 6MWT MCID benchmarks."),
    ]
...
def main() -> None:
    ...
    print("\n  Running delta-adjusted pattern-mixture tipping analysis...")
    delta_summary_df, delta_grid_df, delta_bootstrap_df = run_delta_tipping_analysis(
        df,
        ipcw_completion,
        ipcw_tier_results,
    )
    ...
    with pd.ExcelWriter(OUTPUT_XLSX, engine="openpyxl") as writer:
        ...
        delta_summary_df.to_excel(writer, sheet_name="Delta_Tipping_Summary", index=False)
        delta_grid_df.to_excel(writer, sheet_name="Delta_Tipping_Grid", index=False)
        delta_bootstrap_df.to_excel(writer, sheet_name="Delta_Tipping_Boot", index=False)
    ...
    write_combined_report(lasso_results, ipcw_completion, ipcw_tier_results, delta_summary_df, audit_info)
    ...
    if not delta_summary_df.empty:
        print("\n=== DELTA TIPPING SUMMARY ===")
        for _, row in delta_summary_df.iterrows():
            print(
                f"  {row['Model']:<8}  {row['Scenario']:<5}  "
                f"Δtip={_fmt_delta_tip(row['Δ tipping point (m)']):<11}  "
                f"R²@0={row['R² at Δ=0']:.4f}  "
                f"R²@tip={_fmt(row['R² at tipping point'], 4)}"
            )
