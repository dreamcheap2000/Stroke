...
6MWT4 prediction.  Two key improvements over the 20260831_LASSO_1047 baseline:
  ...
  2.  IPCW section uses ONE well-specified completion model (the richest
      ...
      applied uniformly across all retained Step 1 model-based Ridge regressions.
      This is consistent with conventional IPCW practice, avoids conflating
      "model effect on outcome" with "model effect on weighting", and prevents
      re-deriving a new completion model per model.
...
INPUT_XLSX = ROOT / "20260903_Comprehensive_2139.xlsx"
...
FUNCTIONAL_AUDIT_SPECS = [
    ...
    },
    {
        ...
        "valid_max": 56,
        "auto_clear_zero_t2": True,
        "note": "BBS2=0 with non-null change was treated as no T2 assessment per task request.",
    },
    {
        "name": "MRS",
        "t1": "MRS1",
        ...
        "valid_max": 6,
        "auto_clear_zero_t2": True,
        "note": "MRS2=0 with non-null change was treated as no T2 assessment per task request.",
    },
    {
        "name": "FOIS",
        "t1": "FOIS1",
    ...
    },
    {
        ...
        "valid_max": 30,
        "auto_clear_zero_t2": True,
        "note": "MNA2=0 with non-null change was treated as no T2 assessment per task request.",
    },
    {
        "name": "IADL",
        "t1": "IADL1",
        ...
        "valid_max": 8,
        "auto_clear_zero_t2": True,
        "note": "IADL2=0 with non-null change was treated as no T2 assessment per task request.",
    },
    {
        "name": "FuglUE",
        "t1": "FuglUE1",
        ...
        "valid_max": 66,
        "auto_clear_zero_t2": True,
        "note": "FuglUE2=0 with non-null change was treated as no T2 assessment per task request.",
    },
    {
        "name": "FuglSEN",
        "t1": "FuglSEN1",
        ...
        "valid_max": 44,
        "auto_clear_zero_t2": True,
        "note": "FuglSEN2=0 with non-null change was treated as no T2 assessment per task request.",
    },
    {
        "name": "EuroQoL5D",
        "t1": "EuroQoL5D1",
        ...
        "valid_max": 15,
        "auto_clear_zero_t2": True,
        "note": "EuroQoL5D2=0 with non-null change was treated as no T2 assessment per task request.",
    },
]
...
def _executive_bullets(results: list[dict]) -> list[str]:
    ...
    model2 = next((r for r in results if r["model_name"].startswith("Model 2:")), None)
    model3 = next((r for r in results if r["model_name"].startswith("Model 3:")), None)
    model5 = next((r for r in results if r["model_name"].startswith("Model 5:")), None)
    reference = next((r for r in results if "reference" in r["model_name"].lower()), None)
    weakest = min(results, key=lambda x: x["cv_r2"])
    ...
    if reference is not None:
        ...
        bullets.append(
            f"The weakest retained model was {weakest['model_name']} (CV R² {weakest['cv_r2']:.4f}, MAE {weakest['cv_mae']:.2f} m)."
        )
...
def write_combined_report(
    ...
    intro = doc.add_paragraph(
        ...
        "applied uniformly across all retained models — consistent with conventional IPCW "
        "practice."
    )
    _small_para(intro, 9)
    ...
    if not manual_corrections.empty:
        doc.add_paragraph()
        cr = [["ID", "Column", "Original value", "Corrected value", "Reason"]]
        for _, row in manual_corrections.iterrows():
            cr.append([
                int(row["ID"]) if pd.notna(row["ID"]) else "",
                row["Column"],
                str(row["Original_Value"]),
                str(row["Corrected_Value"]),
                row["Reason"],
            ])
        _add_table_to_doc(doc, cr)
    ...
    ipcw_intro = doc.add_paragraph(
        ...
        "all retained model-based Ridge regressions.\n\n"
        "Rationale for single completion model (per conventional IPCW practice):\n"
        "• Re-deriving a separate completion model per model conflates 'model effect on "
        "outcome' with 'model effect on weighting', making it difficult to isolate "
        "model-specific predictive value from model-specific selection bias.\n"
    ...
    # Per model/scenario results
    for res in ipcw_model_results:
        doc.add_paragraph()
        h = doc.add_paragraph()
        h.add_run(f"{res['model_name']} | {res['scenario_label']} Scenario").bold = True
    ...
    meth_items = [
        ...
        ("Single IPCW Completion Model",
         ...
         "each model/tier using only that set's features.  This creates a methodological inconsistency: "
         "the weighting model changes across model sets, so differences in weighted performance partly "
         ...
         "across all retained model regressions.  This is consistent with the standard causal "
...
def main() -> None:
    ...
    retained_model_specs = [
        (name, MODEL_SPECS[name]) for name in RETAINED_STEP_MODELS if name in MODEL_SPECS
    ]
    for i, (model_name, candidate_features) in enumerate(retained_model_specs):
    ...
    for model_name, model_features in retained_model_specs:
        valid_model_features = _filter_existing(model_features, df)
        if not valid_model_features:
            continue

        print(f"\n  {model_name} ({len(valid_model_features)} features)")
        ...
        for scenario in SCENARIOS:
            ...
            if model_name.startswith("Model 8:") and scenario == "6MWT_Worst_Scenario":
            ...
            model_token = model_name.split(":", 1)[0].replace(" ", "")
    ...
    write_combined_report(lasso_results, ipcw_completion, ipcw_model_results, audit_info)
