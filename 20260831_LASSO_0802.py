...
20260826_5_Tiers_1952.py:
    ...
    Model 2: demographic + stroke info + comorbidities + acute complications
             + NIHSS out + functional T1 WITH gait speed imputed
    Model 3: demographic + stroke info + comorbidities + acute complications
             + NIHSS out + functional T1 WITHOUT gait speed imputed
    Model 4: demographic + BBS1 + gait speed 1 imputed
    Model 5: demographics + functional T1 + T1T2 improvement
    Model 6: demographic + stroke info + comorbidities + acute complications
             + NIHSS out  [original Model 2 – placed last as reference]

Outputs (written to repo root):
    20260831_LASSO_0802.docx  – summary report per model
    20260831_LASSO_0802.xlsx  – full dataset with per-patient LASSO predictions appended
...
OUTPUT_DOCX = ROOT / "20260831_LASSO_0802.docx"
OUTPUT_XLSX = ROOT / "20260831_LASSO_0802.xlsx"
...
def main() -> None:
    ...
    Orchestration:
    1. Load data.
    2. For each of the 6 model specs, filter predictors to existing columns,
