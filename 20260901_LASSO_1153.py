...
Outputs (written to repo root):
    20260901_LASSO_1153.docx  – combined six-model LASSO + IPCW report
    20260901_LASSO_1153.xlsx  – full dataset with per-patient predictions

Author: auto-generated 2026-09-01
"""
...
OUTPUT_DOCX = ROOT / "20260901_LASSO_1153.docx"
OUTPUT_XLSX = ROOT / "20260901_LASSO_1153.xlsx"
...
def fit_bootstrap_lasso(
    ...
    model5_restrict : bool
        ...
        QUALIFYING_REHAB_LOS ('21-42 days' or '>42 days').  T1T2_Change
        values for patients outside these groups are not imputed.
    """
...
def write_combined_report(
    ...
    title.add_run(
        "Combined LASSO + IPCW Report for 6MWT4 Prediction – 2026-09-01"
    ).bold = True

    intro = doc.add_paragraph(
        "This report combines two analyses:\n"
        "(1) Bootstrap LASSO regression models (no IPCW) predicting 6MWT4 as a "
        "continuous outcome — six models with updated Model 5 restricted to "
    ...
    intro2 = doc.add_paragraph(
        f"Six bootstrap LASSO models predicting 6MWT4. "
        ...
        "Model 5 update (2026-09-01): T1T2_Change variables are only meaningful for "
        "patients with sufficient rehabilitation exposure. Model 5 is therefore "
    ...
    comp_heading.add_run("Executive comparison across the six models").bold = True
    comparison_rows = [["Model", "N", "Predictors", "Stable", "CV R²", "CV MAE (m)", "Train-CV R² gap"]]
    for r in sorted(lasso_results, key=lambda x: x["cv_r2"], reverse=True):
...
def main() -> None:
    ...
    for i, (model_name, candidate_features) in enumerate(MODEL_SPECS.items()):
        ...
        is_model5 = model_name.startswith("Model 5:")
        if is_model5 and "Rehab_LOS_Category" in df.columns:
            n_qualifying = int(
                df["Rehab_LOS_Category"].isin(QUALIFYING_REHAB_LOS).sum()
            )
            print(f"  Model 5: restricted to Rehab LOS ∈ {QUALIFYING_REHAB_LOS} "
                  f"({n_qualifying:,} patients qualify)")

        res = fit_bootstrap_lasso(
            df, valid_features, model_name,
            model_seed=RANDOM_STATE + i,
            model5_restrict=is_model5,
        )
    ...
    for tier_name, tier_features in IPCW_TIERS.items():
        ...
        for scenario in SCENARIOS:
            ...
            binary_result = select_binary_classifier(df, valid_tier, scenario)
    ...
    for r in lasso_results:
        ...
        if r["model_name"].startswith("Model 5:"):
    ...
    print("\n=== LASSO SUMMARY ===")
    for r in sorted(lasso_results, key=lambda x: x["cv_r2"], reverse=True):
        print(f"  {r['model_name'][:60]:<60}  N={r['n_patients']:>4}  CV R²={r['cv_r2']:.4f}  MAE={r['cv_mae']:.2f}")

    print("\n=== IPCW SUMMARY ===")
    for r in ipcw_tier_results:
        ci = r["bootstrap_ci"]
        print(
            f"  {r['tier']:<8} {r['scenario_label']:<8}  "
