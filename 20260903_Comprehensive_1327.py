#!/usr/bin/env python3
"""
20260903_Comprehensive_1327.py
======================
...
6MWT4 prediction.  Two key improvements over the 20260831_LASSO_1047 baseline:
  ...
  2.  IPCW section uses ONE well-specified completion model (the richest
      ...
      applied uniformly across all five outcome-tier Ridge regressions.
...
Outputs (written to repo root):
    20260903_Comprehensive_1327.docx  – combined LASSO + IPCW report
    20260903_Comprehensive_1327.xlsx  – full dataset with per-patient predictions

Author: auto-generated 2026-09-03
"""
...
OUTPUT_DOCX = ROOT / "20260903_Comprehensive_1327.docx"
OUTPUT_XLSX = ROOT / "20260903_Comprehensive_1327.xlsx"
...
FUNCTIONAL_AUDIT_SPECS = [
    ...
    },
    {
        ...
        "valid_min": 0,
        "valid_max": 56,
        "auto_clear_zero_t2": False,
        "note": "BBS=0 can be a true score; zero-T2 rows were audited but not blanket-cleared.",
    },
    {
        "name": "MRS",
        "t1": "MRS1",
        ...
        "valid_max": 6,
        "auto_clear_zero_t2": False,
        "note": "Negative MRS change can reflect clinical improvement because lower is better.",
    },
    {
        "name": "FOIS",
        "t1": "FOIS1",
    ...
    },
    {
        ...
        "valid_max": 30,
        "auto_clear_zero_t2": False,
        "note": "Zero-T2 rows and out-of-range values were audited; impossible values were cleared.",
    },
    {
        "name": "IADL",
        "t1": "IADL1",
        ...
        "valid_max": 8,
        "auto_clear_zero_t2": False,
        "note": "IADL=0 can be a true score; zero-T2 rows were audited but not blanket-cleared.",
    },
    {
        "name": "FuglUE",
        "t1": "FuglUE1",
    ...
    },
    {
        ...
        "valid_max": 44,
        "auto_clear_zero_t2": False,
        "note": "Zero-T2 rows were audited; FuglSEN3 also contains later-visit outliers to review separately.",
    },
    {
        "name": "EuroQoL5D",
        "t1": "EuroQoL5D1",
        ...
        "valid_max": 15,
        "auto_clear_zero_t2": False,
        "note": "Zero-T2 rows were audited but not blanket-cleared because 0 may be a true score.",
    },
]
...
# ─────────────────────────────────────────────────────────────────────────────
# IPCW tiers and LASSO model definitions
# ─────────────────────────────────────────────────────────────────────────────
...
BASE_MODEL_SPECS: dict[str, list[str]] = {
    ...
    "Model 5: demographics + functional T1 + T1T2 improvement (Rehab LOS ≥21 days only)": (
...
TIER_LASSO_MODEL_SPECS: dict[str, list[str]] = {
    "Model 7 (Tier 1 predictors): demographic + functional T1 (with gait speed imputed)": IPCW_TIERS["Tier 1"],
    "Model 8 (Tier 2 predictors): demographic + functional T1 (without gait speed imputed)": IPCW_TIERS["Tier 2"],
    "Model 9 (Tier 3 predictors): demographic + stroke info + comorbidities + acute complications + functional T1 (with gait speed imputed)": IPCW_TIERS["Tier 3"],
    "Model 10 (Tier 4 predictors): demographic + stroke info + comorbidities + acute complications + functional T1 (without gait speed imputed)": IPCW_TIERS["Tier 4"],
    "Model 11 (Tier 5 predictors): demographic + functional T1 (with gait speed imputed) + T1T2 improvement (Rehab LOS ≥21 days only)": IPCW_TIERS["Tier 5"],
}

MODEL_SPECS: dict[str, list[str]] = {**BASE_MODEL_SPECS, **TIER_LASSO_MODEL_SPECS}
...
MODEL_PRED_COLS = {
    ...
    "Model 5: demographics + functional T1 + T1T2 improvement (Rehab LOS ≥21 days only)": "LASSO_Pred_Model5",
    "Model 6 (reference – original Model 2): demographic + stroke info + comorbidities + acute complications + NIHSS out": "LASSO_Pred_Model6",
    "Model 7 (Tier 1 predictors): demographic + functional T1 (with gait speed imputed)": "LASSO_Pred_Model7",
    "Model 8 (Tier 2 predictors): demographic + functional T1 (without gait speed imputed)": "LASSO_Pred_Model8",
    "Model 9 (Tier 3 predictors): demographic + stroke info + comorbidities + acute complications + functional T1 (with gait speed imputed)": "LASSO_Pred_Model9",
    "Model 10 (Tier 4 predictors): demographic + stroke info + comorbidities + acute complications + functional T1 (without gait speed imputed)": "LASSO_Pred_Model10",
    "Model 11 (Tier 5 predictors): demographic + functional T1 (with gait speed imputed) + T1T2 improvement (Rehab LOS ≥21 days only)": "LASSO_Pred_Model11",
}
...
MODEL_EXPLAINERS = {
    ...
    "Model 7 (Tier 1 predictors): demographic + functional T1 (with gait speed imputed)": (
        "Tier 1 predictor set replicated in Step 1 LASSO for direct comparability with the five-tier IPCW framework."
    ),
    "Model 8 (Tier 2 predictors): demographic + functional T1 (without gait speed imputed)": (
        "Tier 2 predictor set replicated in Step 1 LASSO to isolate the impact of removing imputed gait speed."
    ),
    "Model 9 (Tier 3 predictors): demographic + stroke info + comorbidities + acute complications + functional T1 (with gait speed imputed)": (
        "Tier 3 predictor set replicated in Step 1 LASSO, combining broad clinical burden with functional T1 including gait speed."
    ),
    "Model 10 (Tier 4 predictors): demographic + stroke info + comorbidities + acute complications + functional T1 (without gait speed imputed)": (
        "Tier 4 predictor set replicated in Step 1 LASSO as the gait-speed-removed counterpart to Tier 3."
    ),
    "Model 11 (Tier 5 predictors): demographic + functional T1 (with gait speed imputed) + T1T2 improvement (Rehab LOS ≥21 days only)": (
        "Tier 5 predictor set replicated in Step 1 LASSO with Rehab LOS ≥21-day restriction for clinically valid T1T2 change interpretation."
    ),
}
...
def audit_and_clean_t1t2_data(df: pd.DataFrame) -> tuple[pd.DataFrame, dict]:
    ...
    Applied cleaning rules are intentionally conservative:
      • BI_T1T2_Change is blanked whenever BI2 == 0 (task-defined as no BI T2 assessment).
...
def fit_weighted_ridge(
    ...
    The weights passed in are the SAME across all tiers (single completion
...
def _executive_bullets(results: list[dict]) -> list[str]:
    ...
    reference = next(r for r in results if "reference" in r["model_name"].lower())
    ...
    model5 = next(r for r in results if r["model_name"].startswith("Model 5:"))
    return [
        (
        ...
        (
            ...
            f"(ΔR² {model3['cv_r2'] - model2['cv_r2']:+.4f})."
        ),
        (
...
def write_combined_report(
    ...
    title.add_run(
        "Comprehensive LASSO + IPCW Report for 6MWT4 Prediction – 2026-09-03"
    ).bold = True

    intro = doc.add_paragraph(
        "This report combines two analyses:\n"
        "(1) Bootstrap LASSO regression models (no IPCW) predicting 6MWT4 as a "
        "continuous outcome — 11 models including five added tier-matched predictor "
        "sets, with updated Model 5 restricted to "
        "patients with sufficient rehabilitation exposure (Rehab LOS ≥21 days).\n"
        "(2) IPCW-weighted Ridge regression across five outcome tiers, using a "
    ...
    if bi_row is not None:
        audit_intro = doc.add_paragraph(
            ...
            f"{int(bi_row['Rows_Cleared_Zero_T2_Rule'])} rows had BI2=0 and were blanked because BI2=0 "
            "was interpreted as no T2 BI assessment. Remaining negative BI change values therefore reflect "
            "observed deterioration rather than missing-T2 artifacts."
        )
        _small_para(audit_intro, 9)

    summary_rows = [[
        "Assessment", "Change rows", "Negative before", "T2=0 rows",
        "Cleared zero-T2", "Cleared out-of-range", "Negative after",
    ]]
    ...
    intro2 = doc.add_paragraph(
        f"Eleven bootstrap LASSO models predicting 6MWT4. "
        ...
        "T1T2-model update (2026-09-03): T1T2_Change variables are only meaningful for "
        ...
        "In addition, BI_T1T2_Change is blanked whenever BI2=0, FOIS_T1T2_Change is blanked whenever FOIS2=0, "
    ...
    ipcw_intro = doc.add_paragraph(
        ...
        "all five outcome-tier Ridge regressions.\n\n"
    ...
    for res in ipcw_tier_results:
        ...
        _add_table_to_doc(doc, [
            ...
            ["Tier feature count", len(res["features"])],
        ])
    ...
    meth_items = [
        ("Model 5 — Rehab LOS Restriction",
         ...
         "left blank and not imputed. In this update, BI_T1T2_Change is also blanked whenever BI2=0, "
        ...
        ("Single IPCW Completion Model",
         ...
         "the weighting model changes across tiers, so differences in weighted performance partly "
         ...
         "across all five tier regressions.  This is consistent with the standard causal "
...
def main() -> None:
    ...
    all_candidate_cols = list({c for cols in MODEL_SPECS.values() for c in cols})
    ...
    for tier_name, tier_features in IPCW_TIERS.items():
        valid_tier = _filter_existing(tier_features, df)
        if not valid_tier:
            continue

        print(f"\n  {tier_name} ({len(valid_tier)} features)")

        for scenario in SCENARIOS:
            scenario_label = scenario.replace("6MWT_", "").replace("_Scenario", "")
            print(f"    Scenario: {scenario_label}", end="  ", flush=True)

            forced_binary_model = None
            if tier_name == "Tier 3" and scenario == "6MWT_Worst_Scenario":
                forced_binary_model = "LogisticRegression(class_weight='balanced', solver='liblinear')"
            binary_result = select_binary_classifier(
                df,
                valid_tier,
                scenario,
                forced_model_name=forced_binary_model,
            )
            regression_result = fit_weighted_ridge(df, valid_tier, ipcw_completion, binary_result)
            ...
            ci = bootstrap_ci_ipcw(
                df, completion_features, valid_tier, scenario,
                n_bootstrap=N_BOOTSTRAP_IPCW,
            )
            print(f"R² CI=[{ci['r2_ci_lo']:.4f}, {ci['r2_ci_hi']:.4f}]")

            tier_token = tier_name.replace(" ", "_")
            walk_col = f"IPCW_Walk_{tier_token}_{scenario_label}"
            pred_col = f"IPCW_6MWT4_{tier_token}_{scenario_label}"
